...
This document is intended to be used as an "open lab notebook" for an fNIRS study on dual-task walking, analyzing an existing dataset, the Park-MOVE dataset collected between 2021 and 2023 [@franzen2023]. Note that while the data is in a central repository, access to the data is restricted and regulated by data transfer agreements.
...
| 6 | 2025-04-25 | Model statistics and formatting |
...
The indirect [@hamacher2015; @herold2017] or executive locomotor [@clark2015; @lafougere2010] pathway involving the prefrontal cortex is thought to be involved in dual-task gait impairments. Specifically, this pathway may be relied upon more heavily in people with dual-task impairments due to both reduced locomotor automaticity and increased executive demands of the concurrent task.
...
# Split into groups and get variables to plot
...
data_oa_c2 <- data_OA %>% filter(Condition == "C2_ST_walk") %>% select(step_time_var = st_step_time_var, Beta, group)
data_oa_c3 <- data_OA %>% filter(Condition == "C3_DT_walk") %>% select(step_time_var = dt_step_time_var, Beta, group)
data_pd_c2 <- data_PD %>% filter(Condition == "C2_ST_walk") %>% select(step_time_var = st_step_time_var, Beta, group)
data_pd_c3 <- data_PD %>% filter(Condition == "C3_DT_walk") %>% select(step_time_var = dt_step_time_var, Beta, group)
...
The model statistics come from the underlying GroupStats model in the NIRS toolbox, where an ANOVA was run on the linear model with the function anova(model, 'summary'), see [MATLAB documentation](https://se.mathworks.com/help/stats/linearmodel.anova.html) and relevant [fork commit](https://github.com/alkvi/nirs-toolbox-fork/commit/402acef1f4c1dec256a71b322b267c69709760d1).
...
ggplot(subject_roi_data_combined, aes(x = step_time_var, y = Beta)) +
  ...
  labs(x = "Step Time Variability (ms)", y = "Beta", title = "Correlation between Beta and Step Time Var") +
  stat_cor(label.y = 11) +
  theme_light() +
